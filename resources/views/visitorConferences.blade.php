...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-7">
                <form action="{{ route('home.search') }}" method="post" class="form_search"> @method('GET') @csrf
                        ...
                        <div class="pt-2" style="display: flex; flex-direction: row; justify-content: space-between">
                            <div class="card bg-dark mb-2" style="width:85%">
                                <div class="card-body con" data-title="{{ $c->title }}" data-description="{{ $c->description }}" data-id="{{$c->id}}"
                                     ...
                                     data-review="{{$c->review_at}}" data-decision="{{$c->decision_at}}" data-finalsubmition="{{ $c->final_submission_at }}" data-final="{{ $c->final_at }}" data-conid="{{$c->id}}">
                            ...
                            <div class="card mb-2">
                                ...
                                <div class="card-body bg-light conference_tr" data-title="{{ $c->title }}" data-description="{{ $c->description }}" data-id="{{$c->id}}"
                                     data-created="{{ $c->created_at }}" data-submission="{{ $c->submission_at }}" data-assigment="{{ $c->assigment_at }}"
                                     data-review="{{$c->review_at}}" data-decision="{{$c->decision_at}}" data-finalsubmition="{{ $c->final_submission_at }}" data-final="{{ $c->final_at }}">
                                    <div class="dates mt-3 mb-5" data-toggle="tooltip" data-placement="left" data-html="true"
                                    ...
                                    <div class="members mt-5" data-toggle="tooltip" data-placement="left" data-html="true"
        ...
        <script>
            ...
            $(document).ready(function () {
                $(".con").click(function () {
                    ...
                    @guest
                        url = "/home/" + encodeURIComponent("{{ $type }}") + "/" + encodeURIComponent(conid);
                    @else
                        @if(Auth::User()->system_role==='Admin')
                            url = "/myconferences/Visitor/" + encodeURIComponent(conid);
                        @endif
                        url = "/myconferences/" + encodeURIComponent("{{ $type }}") + "/" + encodeURIComponent(conid);
                    @endguest
